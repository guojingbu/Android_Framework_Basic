...
public class DeviceUtil {
    ...
    static {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            ...
            try {
                ...
                LangHelperUtil.close(fileInputStream);
            }
        }
     ...
     * @param devicesCode
     */
    private static void saveLocalDeviceCode(String devicesCode) {
        if (EnvironmentUtil.Storage.isExternalStorageWritable()) {
            File sdCardRootDirectory = Environment.getExternalStorageDirectory();
            File file = new File(sdCardRootDirectory, "/android/v2x-a1.text");
            FileUtils.store(devicesCode, file.getAbsolutePath());
        }
    }
     ...
     * @return
     */
    private static String getLocalDeviceCode() {
        String deviceCode = "";
        if (EnvironmentUtil.Storage.isExternalStorageWritable()) {
            File sdCardRootDirectory = Environment.getExternalStorageDirectory();
            File file = new File(sdCardRootDirectory, "/android/v2x-a1.text");
            if (file.exists()) {
                deviceCode = FileUtils.load(file.getAbsolutePath());
            }
        }
        return deviceCode;
    }
