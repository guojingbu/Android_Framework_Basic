...
//        canvas.drawPath(mPathAlphaArc, mPaintAlpha);
        ...
        if(new PathMeasure(mPathArc, false).getLength() >
                Math.max(getPaintWidth(mPaintText, percent), getPaintWidth(mPaintText, ""))) {
...
//            canvas.drawText(name, mTextPoint[0], mTextPoint[1], mPaintText);
            canvas.drawText(percent, mTextPoint[0], mTextPoint[1] + getPaintHeight(mPaintText, percent), mPaintText);
        }
    }
