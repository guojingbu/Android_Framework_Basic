package com.yesway.common_lib.widget.pie;
...
public class PieDataView extends View implements BasePieView.IPieDataView {
    ...
    private void initView() {
        ...
        mPaintText.setColor(getContext().getResources().getColor(R.color.black_2));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //计算每行的高度
        float rh = getPaintHeight(mPaintText, mNames==null||mNames.length==0?"":mNames[0] + " " + mValues[0]);
        //当前行
        int row = 0;
        //文字和色块之间的距离
        int spacing = dp2px(5);
        ...
        for (int i = 0; i < mValues.length; ) {
            ...
            for (float x : mPointXCoordinate) {
                ...
                //绘制文字
                //String text = mShowFormat.replace("{name}",mNames==null||mNames.length==0?"": mNames[i]).replace("{value}",mValueFormat.format(mValues[i]));
                String text = mShowFormat.replace("{name}",mNames==null||mNames.length==0?"": mNames[i]).replace("{value}","");
